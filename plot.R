...
simu <- 4
plot( d$gen[d$simu==simu], d$mean[d$simu==simu], type="l" )

png( paste(inFile,".png",sep=""), width=900, height=600 )
par( mar=c(5,5,3,2), font=2, font.axis=2, font.lab=2, cex=1.5, lwd=2 )
plot( unique(d$gen), seq(0,max(d$mean),length.out=length(unique(d$gen))),
     type="n",
...
for( s in unique(d$simu) ){
  points( unique(d$gen[seq(1,max(d$gen),10)]),
         d$mean[d$simu==s][seq(1,max(d$gen),10)],
         type="l", lwd=0.5 )
}
abline( h=mean(d$mean[d$gen==max(d$gen)]), lty=2 )
dev.off()
